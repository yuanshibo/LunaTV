...
/**
 * Extracts a clean JSON string from a raw AI response that might be wrapped in a markdown code block.
 * @param rawResponse The raw string response from the AI.
 * @returns A clean JSON string.
 */
function extractJsonFromString(rawResponse: string): string {
  if (!rawResponse) {
    return '';
  }
  ...
  // Return the original string if no markdown block is found, after trimming.
  return rawResponse.trim();
}

/**
 * A robust JSON parsing function that cleans the AI response before parsing.
 * @param rawResponse The raw string response from the AI.
 * @returns The parsed JSON object, or throws an error if parsing fails.
 */
function parseJsonFromAiResponse(rawResponse: string): any {
  const cleanJsonString = extractJsonFromString(rawResponse);
  if (!cleanJsonString) {
  ...
  try {
    ...
    console.error("Failed to parse cleaned JSON string:", cleanJsonString);
    throw new Error(`Invalid JSON format after cleaning: ${(error as Error).message}`);
  }
}
...
export async function generateContent(prompt: string, isJson = false): Promise<any> {
  ...
  if (provider === 'gemini' && geminiApiKey) {
    try {
      ...
      return isJson ? parseJsonFromAiResponse(result) : result;
    } catch (error) {
      console.error(
  ...
  try {
    ...
    return isJson ? parseJsonFromAiResponse(data.response) : data.response;
  } catch (error) {
